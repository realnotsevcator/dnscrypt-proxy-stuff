...
def read_text_from_url(url: str, *, timeout: float = 30.0) -> str:
    request = urllib.request.Request(
        url,
        headers={
            # Present a common browser user agent so the request blends in with
            # regular traffic and avoids HTTP 403 responses from the host.
            "User-Agent": (
